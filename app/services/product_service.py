...
class ProductService:
    ...
    async def set_rent_period(
        ...
        product.rent_expires_at = datetime.now(timezone.utc) + timedelta(days=30 * months)
    ...
    async def approve_product(
        ...
        # Get approval fee from settings
        approval_fee = await settings_service.get_setting_float(db, "shop_approval_fee", 5.0)

        # Check shop balance
        if shop.balance < approval_fee:
            raise ValueError(f"Недостаточно средств на балансе магазина. Требуется: ${approval_fee}, доступно: ${shop.balance}")

        # Charge shop
        shop.balance -= approval_fee
    ...
    async def reject_product(
        ...
        # Delete product images when rejected
        if product.images:
            logger.info(f"Deleting {len(product.images)} images for rejected product {product_id}")
            product.images = []
        ...
        await db.commit()
        await db.refresh(product)
        logger.info(f"Product rejected: {product_id}, images cleared")
        return product
