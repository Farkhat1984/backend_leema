...
async def google_callback(
    ...
    try:
        ...
        if not user_info:
            return HTMLResponse(content=f"""
                ...
                <body>
                    ...
                    <a href="/shop.html">Вернуться на главную</a>
                </body>
                </html>
            """, status_code=401)

        # Determine account type based on client and requested type
        if client_type == "mobile":
            ...
            # Web - check existing accounts
            # First, check if user exists and is admin
            user = await user_service.get_by_google_id(db, user_info["google_id"])
            if user:
                role_str = user.role.value if hasattr(user.role, 'value') else str(user.role)
                if role_str == 'admin':
                    # Admin user - allow access to admin panel
                    access_token = create_access_token({"user_id": user.id, "role": role_str})
                    account_type = 'admin'
                else:
                    # Regular user exists but not admin - check if shop exists
                    shop = await shop_service.get_by_google_id(db, user_info["google_id"])
                    if shop:
                        # Shop exists - use shop account
                        access_token = create_access_token({"shop_id": shop.id})
                        account_type = 'shop'
                    else:
                        # User exists but not admin and no shop - create shop
                        shop_data = ShopCreate(
                            google_id=user_info["google_id"],
                            email=user_info["email"],
                ...
                if shop:
                    ...
                    # No user, no shop - create new shop by default for web
                    shop_data = ShopCreate(
                        google_id=user_info["google_id"],
                        email=user_info["email"],
                        shop_name=user_info["name"],
        ...
        return HTMLResponse(content=f"""
            ...
            <body>
                ...
                <script>
                    ...
                    const refreshToken = '{refresh_token if client_type == "mobile" else ""}';
                    const accountType = '{account_type}';
                    const userData = {json_lib.dumps(user_data_json) if client_type == "mobile" else "{}"};
                    ...
                    if (clientType === 'mobile') {{
                        ...
                        const flutterOrigin = document.referrer || 'http://localhost:57063/';
                        ...
                        // Web admin panel - save to localStorage and redirect
                        localStorage.setItem('token', token);
                        localStorage.setItem('accountType', accountType);
                        console.log('[AUTH] Token saved:', token);
                        console.log('[AUTH] Account type:', accountType);
                        console.log('[AUTH] Redirect logic - accountType value:', accountType, 'type:', typeof accountType);

                        setTimeout(function() {{
                            console.log('[AUTH] Executing redirect for accountType:', accountType);
                            if (accountType === 'admin') {{
                                console.log('[AUTH] Redirecting to /admin.html');
                                window.location.href = '/admin.html';
                            }} else if (accountType === 'shop') {{
                                console.log('[AUTH] Redirecting to /shop.html');
                                window.location.href = '/shop.html';
                            }} else {{
                                console.log('[AUTH] Redirecting to /index.html (default)');
