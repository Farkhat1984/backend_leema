...
async def check_expiring_rents():
    ...
    async with async_session_maker() as db:
        try:
            ...
            three_days_ago = datetime.now(timezone.utc) + timedelta(days=2)  # To avoid sending multiple times
            ...
            for product, shop in products_shops:
                days_left = (product.rent_expires_at - datetime.now(timezone.utc)).days
            ...
            result = await db.execute(
                select(Product).where(
                    and_(
                        Product.is_active == True,
                        Product.rent_expires_at.is_not(None),
                        Product.rent_expires_at <= datetime.now(timezone.utc)
                    )
                )
            )
